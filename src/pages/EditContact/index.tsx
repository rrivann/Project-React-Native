...
import {Button, Gap, ModalCustom, TextInput} from '../../components';
import {UpdateContactAction, useAppSelector} from '../../redux';
import {colors, getData, IconFontAwesome, ShowToast} from '../../utils';

const EditContact: FC = ({route, navigation}: any) => {
  const data = route.params;
  const dispatch = useDispatch();
  const {phoneBook} = useAppSelector(s => s);
  const [formData, setFormData] = useState(data);
  const [modalVisible, setModalVisible] = useState(false);
  const [photo, setPhoto]: any = useState(data.image);
  const [photoForDB, setPhotoForDB]: any = useState(data.image);
